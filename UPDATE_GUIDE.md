...
議案データのExcel・CSV取込は「2. 議案を追加する方法」内の「Excel・CSVでまとめて議案情報を取り込む方法」を参照してください。一般質問・報酬データの取込は今後の課題です（詳しくは完了報告の「未完了事項」をご覧ください）。
...
### Excel・CSVでまとめて議案情報を取り込む方法（上級者向け）

議員データと同じ「下書きを作る」取込機能が、議案データにも使えます（TASK-021）。**この機能は「下書きを作る」ためのものです。既存のデータを自動的に書き換えることはありません。個人別賛否（`memberVotes`）はこの機能では取り込めないため、必要な場合は下書き反映後に手作業で追加してください。**

1. `templates/bills-template.xlsx` をコピーして開く
2. 1行目（見出し行）は変更せず、2行目以降に議案情報を入力する
   - 見出し：`id` `会計年度` `会期` `会期ID` `議案番号` `件名` `概要` `提出日` `議決日` `付託委員会` `提出者` `提出者区分` `議決結果` `採決方法` `分類` `トピック` `議案書URL` `審議結果PDF URL` `会議録URL` `最終確認日`
   - `提出者区分` は `mayor`（市長提出）/ `member`（議員提出）/ `committee`（委員会提出）/ `other` のいずれか
   - `議決結果` は `src/types/index.ts` の `BillVoteResult` に定義された値のいずれか（例：`原案可決` `否決` `認定` `同意` `採択` `確認中` など）
   - `議決結果` が `確認中` 以外の場合、`議案書URL` `審議結果PDF URL` `会議録URL` のいずれか1つ以上の入力が必須です（根拠資料のない確定結果は登録できません）
   - `提出日` `議決日` `最終確認日` は `YYYY-MM-DD` 形式
   - `トピック` は複数ある場合、読点（、）またはカンマ（,）で区切る
3. ターミナルで次を実行する

   ```
   npm run import:bills -- 保存したファイルのパス.xlsx
   ```

4. 次のいずれかになります。
   - **エラーが表示された場合**：内容を修正して、もう一度実行してください（この場合、何も反映されていません）
   - **成功した場合**：次の場所に「下書き」が作られます
     - `generated/import-preview/bills.json`：変換されたデータ（プレビュー）
     - `generated/import-preview/bills-diff.txt`：既存データとの違い（新しく増えるID、含まれていないID など）
     - `backups/日時/billVotes.json`：取込前の`src/data/billVotes.json`のバックアップ
5. **`generated/import-preview/bills.json` の中身と、`bills-diff.txt` の差分を必ず人の目で確認してください。**
6. 問題がなければ、確認した内容を手作業で `src/data/billVotes.json` へ反映してください（自動反映はしません）。取り込んだデータには`extractionSource: "manual"`が自動で設定され、手入力データであることが区別されます。
...
## 5-2. 注記欄（`notes` など）の書き方と、内部の番号の扱い

データには、出典や調査の経緯を書く自由記述の欄があります。役割は次のとおりです。

| 欄 | 役割 | 画面に出るか |
| --- | --- | --- |
| `sourceRefs` / `sourceRef` | 出典そのもの（資料名・URL・公表日・確認状況）を項目に分けて記録する | 出る（出典一覧として） |
| `notes` / `note` / `sourceNote` / `termNote` / `definitionNote` | その項目の補足説明・調査の経緯 | 出る（本文として） |
| `verificationNote` | 「いつ・どの資料で・どう確認したか」の作業記録 | 一部のページで出る |
| `generatedFrom` | どのデータから作った値かの記録 | 出ない（内部の監査用） |

出典として使える資料名・URLがあるときは、注記欄に書かずに `sourceRefs` へ入れてください
（後から出典の一覧として表示・検証できるため）。

注記欄に、当サイト内部の番号（`civic-047`・`csp-35`・`fm32` など）やファイル名を書いても構いません。
**出典をたどるために必要なので、後から消さないでください。** 画面に出すときは
「整理番号civic-047」「市政年表データ」のように、市民が読める日本語へ自動で言い換えています
（`src/lib/citizenTermLabels.ts`）。

新しい種類の番号や項目名を注記へ書いた場合は、`npm run build` の最後に実行される
`scripts/test-text-quality.mjs` が「意味の分からない英数字が画面に出ている」ことを検出します。
その場合は、データを消すのではなく `src/lib/citizenTermLabels.ts` へ日本語の言い換えを追加してください。
