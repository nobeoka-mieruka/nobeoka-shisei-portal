...
ユーザーから「Wayback Machineが503でも停止せず、別の資料経路を使って未解決データを埋める」
との指示（7優先度）を受けた。同じURL・同じ資料の重複調査を避けるため、TASKS.md・
blockedTaskClassification.json・archiveMayors.jsonの既存sourceRefs/notesを先に確認して
から着手した。

### 実施内容

- **Priority 1（Wayback再確認）**：`reports/wayback-recovery-queue.json`の先頭3件を
  curlで再確認したが、依然503のため即座にPriority 2へ切り替えた（42件の再試行なし）。
  `lastCheckedAt`/`playbackStatus`をキューへ記録。
- **Priority 2〜3（歴代市長・NDL）**：WebSearch/WebFetchで新規調査したところ、
  mayor-05・06・09・10・13の生没年・出身地が、既存のsourceRefs（Wikipedia、
  2026-08-05〜15時点で個別確認済み）のnotesに**既に記載されているが構造化フィールド
  （birthDate/deathDate/birthplace）には反映されていなかった**ことを発見し、構造化した
  （新規調査ではなく既存確認済み記述の反映）。NDLサーチ（ログインなし）で
  『日本の歴代市長：市制施行百年の歩み 第3巻』（1985年、個人送信サービス対象、
  mayor-04〜12を扱っている可能性）を新規発見しTASK-074のsourceInventoryへ追加。
  房野博（mayor-11）自身の著作『きずな一筆』も発見したが視覚障害者向け音声資料と判明し
  追跡終了。ログインは一切試みていない。
  - 【重要な学び】折小野良一（mayor-10）・青木善祐（mayor-09）双方のWikipedia記事を
    フェッチしたところ、1960年市長選挙の得票差（1,139票）等の詳細が**両方の既存notesに
    既に記録済み**であることが判明した。フレッシュな検索が既存の充実した記録を再発見する
    だけに終わるケースが多く、事前のデータ確認の重要性を再確認した。
- **Priority 4（財政空白年度）**：archiveFiscalYearsの人口データ欠損8年度
  （FY2001-2004・2005・2006-2008）を、既にFY2009以降で使用中の延岡市公式Excel長期系列
  （現住人口及び世帯数の推移）から新規取得し、人口データを26年度中18→26年度（100%）に
  到達させた。FY2011-2016の市債残高（ordinaryAccountLocalBondBalanceYen）が未登録に
  見えた点を調査したところ、**既に「一般・特別・企業会計の合計」という別範囲の値
  （includingEnterpriseAccountsYen）として2026-08-12/16時点で登録済み**であり、
  普通会計ベースの値はそもそも当該年度の広報のべおかに掲載されていないことが判明した
  （真の欠損ではなく集計範囲の違い、既存のdefinitionNoteで既に説明済み）。誤って
  重複登録しそうになったが、既存データの精査により回避した。
- **Priority 5（広報のべおか）**：新規ダウンロードはせず、既存のkohoNobeokaIssues.json・
  kohoOcrSearchIndex.jsonを確認。広報アーカイブは2010年4月号以降のみで、mayor-04〜13
  （〜2006年退任）の時代をカバーしていないことを確認。FY2011-2016の財政状況公表号
  （5月号・11月号）は既に上記の通り登録済みであることを確認した。
- **Priority 6（過去議員）**：今回の調査過程で新規の過去議員候補は発見されなかった
  （0件、大規模クロールは実施していない）。
- **Priority 7（市政年表）**：civicTimelineEvents.jsonの型定義上、歴代市長の就任・退任は
  意図的に対象外（archiveMayors.json/archiveMayorTerms.jsonで別管理）と明記されている
  ことを確認し、今回発見した市長選挙関連の事実（得票差等）は市政年表へは追加しなかった
  （設計上正しい除外、追加不要と判断）。

### 検証結果

各コミットで`validate:data`（errors=0, warnings=14=既存同数）／`validate:finance`
（errors=0, warnings=0, info=8=既存同数、前年度比異常検知0件）／`typecheck`／`lint`／
`build`（2128/2128ルート、validate-seo 0/0、validate-content 0/0）すべて成功。

### コミット・デプロイ

`31daa86`（TASK-085）→`e374897`（TASK-086）、`origin/main`へpush予定。

### 次回開始地点

1. `git status`／`git log --oneline -10`／`origin/main`との差分
2. Wayback Machine再生バックエンドの復旧を1回だけ確認
   （`reports/wayback-recovery-queue.md`の手順）
3. 復旧していればP0高信頼度42件を処理
4. していなければユーザーへNDLログイン状況を確認（TASK-074へ新規追加した
   『日本の歴代市長 第3巻』の確認を優先候補として提示）
5. 歴代市長の政策・議案・条例・大型事業データは依然mayor-03（読谷山洋司）以外は
   ほぼ0件のまま（1990年代以前は一次資料がウェブ上にほぼ存在しないため、物理資料調査
   以外での前進は見込みにくいと判断済み、TASK-073参照）
...
UI・表示整合性改善フェーズ（TASK-091〜095）完了後、ユーザー指示によりBLOCKED/RESEARCH_EXHAUSTED
の既存7タスク（TASK-004・016B・032・045・046・047・074）を対象に、4系統（歴代市長／
1999年以前選挙／政治資金／議員経歴）を可能な範囲で並列調査した。

### 実施内容の要旨

- **歴代市長（自分で実施）**：mayor-05〜13を個別に再検証。mayor-06（大島文彦）・mayor-09
  （青木善祐）のWikipedia個人記事に『全国市長銘鑑』『長崎市制五十年史』等の名前付き引用が
  あることを新たに確認し、無出典情報との区別をnotesに明記（verificationStatusは
  needsReviewのまま維持、引用元原本は未確認のため）。mayor-08・mayor-11は再検索したが
  新情報なし。mayor-13の1994・1998年就任日は選挙ドットコムの年別スケジュール一覧
  （新しい確認方法）でも掲載なしを確認。TASK-045（任期空白13件）はNDL『延岡市史』
  1983/1993年版の非ログイン部分を再確認したが新情報なし。データの新規追加は無し（推測禁止）。
- **1999年以前選挙（並列agent）**：NDLサーチの「本文全文検索」（書誌名検索ではなく全文
  フレーズ検索、今回初めて試行）で『延岡市史 下巻』『延岡商工会議所四十年史』等の書誌的
  手がかりを新規発見。Web Archive（Wayback Machine）・総務省e-Statも確認したが1999年以前の
  候補者別データはいずれも存在しないことを確認。`electionResults.json`への追加は無し。
- **政治資金（並列agent）**：令和7年分は引き続き未公表（変化なし）。21団体の年度カバレッジを
  棚卸しし漏れがないことを確認。繰越額未登録3件へpdfjs-distでテキスト層抽出を再試行したが
  画像スキャンと再確認（推測せず）。
- **議員経歴17件（並列agent）**：選挙公報（候補者本人記載の経歴欄）・Web Archiveという新しい
  角度で調査を試みたが、選挙公報PDFの視覚確認に時間がかかり約45分で完了の見込みが立たず、
  セッション時間予算を優先して安全に中断した。`members.json`への変更は無し。選挙公報という
  着眼点自体は有望なため、次回専用セッションでの再試行候補として記録した。
- **blocked分類の精緻化**：`blockedTaskClassification.json`に`nextActionCategory`
  （needs_ndl_login／needs_human_review／needs_offline_library／source_not_published／
  research_exhausted／conflicting_sources／waiting_for_publication／actionable_now）を
  追加。既存の`status`（5状態）・UI表示ロジックは変更せず、7タスク全件に付与した。
- ユーザーが提示した`attachment/2703.pdf`は、既存のTASK-047（koho-2010-04、構造破損PDF）と
  同一ファイルであることを確認。再解析も同じ結論（新情報なし）。

### 検証結果

`validate:data`（errors=0, warnings=14＝既存と同数）、`validate:sources`（0/0）、
`validate:completeness`（0/0）、`validate:finance`（0/0）、`validate:political-funds`（0/0）、
`validate:freshness`（0/0）、`typecheck`、`lint`、`test`（26件成功）、`build`
（2122/2122ルート、`validate-seo` 0/0、`validate-content` 0/0）、`release-check.mjs`
（failures=0、既存の助言的warning 2件のみ）すべて成功。新規warning・errorなし。

### コミット・デプロイ

`936c8c1`→`64b9e69`（歴代市長・選挙・政治資金）、`origin/main`へpush済み。
続けて`TASK-032`関連の追加記録・本メモをまとめて次のコミットでpush予定。

### 引き継ぎ事項

- TASK-032（議員経歴17名）：選挙公報の視覚確認は今回未完了。次回、時間を十分確保した上で
  claude-in-chromeによる選挙公報PDFの候補者経歴欄確認を再試行する価値がある。
- TASK-046：NDLサーチ本文全文検索で見つかった『延岡市史 下巻』（pid/9774712）・
  『延岡商工会議所四十年史』の該当箇所は、NDL個人送信サービスへのログイン後に確認すれば
  前進する可能性がある。
- TASK-045・074：同じくNDL『延岡市史』1983/1993年版のログイン後確認が再開条件。

## 2026-08-15：表示矛盾解消・総合改善（ユーザー指示20項目、TASKS.md TASK-075）

ユーザーから「延岡市政見える化ポータル 総合改善・表示矛盾解消」の指示（新規大型機能では
なく、既存データを市民が誤解せず信頼して利用できる状態に仕上げることを最優先とする20項目、
チャット上ではTASK-071〜090と呼称）を受け、上から順番に1項目ずつ着手した。

### 実施内容の要旨

詳細はTASKS.md「TASK-075 表示矛盾解消・総合改善（ユーザー指示TASK-071〜090、Phase140）」を
参照。要旨は以下のとおり。

- **表示矛盾の解消**：議案賛否「26/26名」表示（議員紐付け数と議案確認数を分離）、
  「議案3件」対「議案1,177件」（詳細アーカイブと採決データベースを名称で分離）、
  人物ページ「関連資料0件」（一般質問・議案賛否等の活動が別途確認できる場合を区別）を解消
- **表記統一**：0件・未確認・未収録・非公開不存在・調査中の5区分を`dataAvailabilityStatus.ts`
  として新設し共通コンポーネント化。「ランキング」表現を中立的表現へ変更
- **トップページ再設計**：入口化（議員を探す簡易導線）、最近の更新（updateHistory.json
  最新4件）、データ整備状況（/data-statusと同じ計算式、単一スコアは作らない）を新設
- **令和8年6月定例会の一般質問**：延岡市公式サイトで「のべおか市議会だより」第108号
  （令和8年8月1日発行）が同定例会の一般質問を収録して公開済みであることを確認し、
  「質問通告書確認済み→市議会だよりで開催確認済み→公式会議録確認済み」の3段階を導入
  （会議録は自動更新パイプラインの直近チェックでも未公開のまま、未確認を確認済みにはしていない）
- **歴代市長ページ**：収集済みだが未表示だった選挙日（electionDate）を新規表示、
  読み仮名未登録4名に「読み：未確認」を明示
- **財政データ**：欠損年度（基金2009〜2017・2026年度、予算決算2018・2019・2025年度）を
  明示する共通ヘルパーを新設。市債残高は全18年度で確認済みと確認
- **サイト全体監査**：validate系全スクリプト・release-check・check-external-links・
  freshness-reportを再実行し新規の問題が無いことを確認（既存の404 3件は既に注記済み、
  監査レポート内の古いURL残存は現データに実在しない履歴上の残骸と判明）
- **モバイルUI**：claude-in-chrome拡張が本セッション中一貫して未接続だったため、コード監査
  （overflow-x-auto・sm:hidden対応関係）で代替。CompensationPageの横スクロール案内文の
  表示条件が実際の表と逆になっていたバグを修正
- **アクセシビリティ**：主要な絞り込みページの結果件数へaria-live="polite"を追加
- **集計ロジック共通化**：任期の日単位確認・議案提出者区分確認・財政年度別データ有無の
  判定を複数ページで重複実装していたものを共通ヘルパーへ統合（トップページと/data-status
  の数字が将来ズレない構造に）
- **出典・確認日表示**：収集済みだが未表示だったaccessedAt（確認日）を新規共通コンポーネント
  `SourceRefList`へ追加。/finance/funds・/finance/debtに出典セクション自体が無かったため新規追加
- **デザイン統一**：SectionCard・StatCardが既に単一の共有コンポーネントとして使われている
  ことを確認（監査のみ、無理な統一はしていない）

### 検証結果

各コミットごとに`validate:data`（errors=0 warnings=14、既存と同一）／`typecheck`／`lint`
（oxlint、0件）／`test`（26/26）／`build`（prerender 2122/2122）／`validate:seo`
（failures=0）／`validate:content`（errors=0）を実行、すべて成功。最終まとめて
`validate:freshness`／`validate:sources`／`validate:completeness`／`validate:finance`／
`validate:political-funds`／`release-check.mjs`（既存の助言的warning 2件のみ）も再実行し
成功を確認した。billVotes=1177等の既存基準値はリファクタ前後で完全一致。

### コミット・デプロイ

`309555c`→`0a66eee`→`5056fe5`→`21b6ab7`→`27df17b`→`3506df6`→`ab1565d`→`353b853`→
`7ee3d09`→`d061d38`→`7bf606f`→`62d8cb8`→`749ff1b`→`c719476`→`7df8054`→`baeafed`、
すべて`origin/main`へpush済み。Cloudflare PagesのGit連携により自動デプロイされる想定
（本セッションではCloudflare側の反映状況を直接確認するツールに接続していないため、
次回セッション開始時に本番URLでの反映確認を推奨する）。

### 残課題・次回への引き継ぎ

- **claude-in-chrome拡張が本セッション中一貫して未接続**（TASK-076〜083関連の実機視覚
  確認が未実施）。次回セッション開始時に接続を再確認し、375/390/430/768/1280pxでの
  本番URL実機確認（特に本セッションで再設計したホームページ、修正した
  CompensationPage・CompareSimilarMunicipalitiesPageの横スクロール表示）を推奨する
- CouncilDocumentsArchivePage.tsxのsourceRefs表示は、審議結果資料への追加フォールバック
  リンクが配列外にあるため、TASK-087のSourceRefList共通化の対象外とした（既存表示に
  問題はないが、accessedAtの表示強化は未適用のまま）
- CommitteesPage.tsxの委員会種別セクション（常任・議運・特別）は、フィルタ結果が0件の
  場合セクション自体を非表示にする実装のまま（検索結果として0件を隠す妥当な設計と判断し
  変更しなかったが、将来的に「0件」明示が必要と判断されれば見直しの余地あり）
- 従来からの残課題（TASK-046の1999年以前選挙調査、TASK-016Bの政治資金令和7年分、
  歴代市長mayor-04〜14の政策データ収集＝NDLログイン待ち等）は変更なし

## 2026-08-14（同日7回目）：TASK-074 歴代市長データ収集のNDL（国立国会図書館）一次資料調査

TASK-073完了後、ユーザーからNDL利用者登録完了の連絡を受け、「今後の歴代市長・市政史調査
ではNDLを正式な調査経路として使ってほしい」との指示があった。

### 実施内容

NDLサーチ（ndlsearch.ndl.go.jp）で以下を確認した（**ログインは一切行っていない**。
認証情報の入力・保存はコード・Git・ログ・設定ファイルのいずれにも行わず、ログイン画面が
表示された場合は入力欄に触れずその場でページを離れた）：

- **『延岡市史』**（延岡市史編さん委員会編、延岡市、1963年、1101p、請求記号219.6-N873n）：
  TASK-045で既に発見済みだった書誌を再確認し、個人送信サービス対象であることと詳細な
  書誌事項を記録した。
- **『延岡市勢要覧』**：1933（昭和8年版）〜2004年の計12版がNDLサーチに登録されていることを
  新規発見。1949年版は一覧上「インターネットで読める」表示だが、実際に開くとログイン
  画面へ遷移することを確認（ここでも入力はせず離脱）。
- **『政治家人名事典：明治〜昭和 新訂』**（日外アソシエーツ、2003年）：複数の歴代市長の
  Wikipedia記事が出典としているが、紙のみでデジタル化されておらず来館必須と判明。
- **『延岡市議会会議録』**：個別会期が宮崎県立図書館所蔵として書誌登録されている
  （例：1965年＝折小野良一期、1974-75年＝房野博期）。現物閲覧が必要。

いずれも書誌情報（存在・所在・アクセス方法）のみを記録し、本文の内容は一切閲覧・転載
していない。`src/data/blockedTaskClassification.json`へTASK-074として
`sourceInventory`・`reopenConditions`を構造化して記録した（TASK-046と同じパターン）。

### 検証結果

`validate:data`（errors=0 warnings=14、既存と同一。新規追加したBLOCKED件数の整合性
チェックも一致することを確認）／`typecheck`実施。データ内容（市長経歴等）は変更していない
ため、build等は前回コミット時点のまま。

### 現在の残課題（更新）

- **TASK-074（本件）**：ユーザー本人がNDLへログインし、上記資料の本文を確認できれば
  BLOCKED状態を解除し、歴代市長データ（mayor-04〜14）を追加できる。次回セッション開始時に
  ユーザーへログイン状況を確認すること。
- その他の残課題（歴代市長の市政年表追加候補、mayor-02の2014年PDF、Claude in Chrome拡張の
  表示不具合、TASK-046・TASK-016B）は変更なし。

### 次回開始地点

1. `git status`／`git log --oneline -10`／`origin/main`との差分
2. ユーザーにNDLログイン状況を確認し、可能であれば`blockedTaskClassification.json`の
   TASK-074エントリの`reopenConditions`に沿って歴代市長データを追加する
3. 上記が難しい場合は、TASK-073の残課題（市政年表追加候補等）を継続する
