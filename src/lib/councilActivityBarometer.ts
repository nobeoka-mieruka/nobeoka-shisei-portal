...
import petitionIntroducersData from "../data/petitionIntroducers.json";
...
function buildRecordExtras(member: CouncilMember, speeches: CouncilSpeech[]): CouncilActivityRecordExtras {
  ...
  const petitionIntroductions: ActivityRecordListItem[] = petitionIntroducerRecords
    .filter((r) => r.introducerMemberIds.includes(member.id))
    .map((r) => ({
      label: `${r.billNumber} ${r.billTitle}`,
      detail: r.completenessNote,
      url: r.sourceRefs[0]?.url,
      urlLabel: "会議録",
    }));
...
export function getEvidenceAvailabilitySummary(): EvidenceAvailabilityItem[] {
  ...
  const entries: EvidenceAvailabilityEntry[] = [
    ...
    },
    {
      ...
      code: "research_exhausted",
      detail:
        "会議録検索システムの本文・議会だより・議会公式サイトの本会議関係ページ等を調査しましたが、議員別の出席・欠席名簿はウェブ公開の資料に掲載されていません（地方自治法上、会議録原本には出席議員の氏名が記載されるため、原本には記載がある可能性があります）。「資料が見つからない」ことは「全員出席」を意味しません。",
    },
    {
      key: "petition",
      label: "請願・陳情等",
