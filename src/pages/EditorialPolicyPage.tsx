...
const sourceList = [
  ...
  "市長記者会見",
  "政党、会派、政治団体などの公式発表",
  "総務省・宮崎県など国・県の公的機関の資料（e-Statなどの統計を含む）",
  "国立国会図書館（サーチ・デジタルコレクション）などの公的機関の書誌・所蔵情報",
  "Wayback Machine（インターネットアーカイブ）に保存された過去の公式ページ",
  "延岡市立図書館・宮崎県立図書館などが所蔵する郷土資料",
];
  ...
  return (
    <div className="space-y-4 px-4 py-4 sm:px-6">
      ...
      <div className="mx-auto max-w-2xl rounded-2xl bg-white p-5 shadow-e1 sm:p-8">
        ...
        <Section id="research-method" title="古い年代・未公開資料の調べ方">
          <p>
            延岡市議会の会議録検索システムは平成12年（2000年）5月以降のみを収録しているなど、公式サイトだけでは確認できない年代の情報があります。そうした場合、当サイトは次のような公開の一次資料・公的機関の情報を組み合わせて調査しています。
          </p>
          <ul className="list-disc space-y-1 pl-5">
            <li>延岡市史などの自治体刊行物（国立国会図書館デジタルコレクション等で公開されているもの）</li>
            <li>国立国会図書館サーチの書誌情報（所蔵館・請求記号の確認）</li>
            <li>Wayback Machine（インターネットアーカイブ）に保存された、延岡市・延岡市議会の過去の公式ページ</li>
            <li>延岡市立図書館・宮崎県立図書館などの郷土資料（現地での閲覧が必要な場合を含む）</li>
          </ul>
          <p className="font-medium text-slate-900">
            これらの方法で調べても確認できない情報は、推測で埋めません。
          </p>
          <p>
            「確認中」「未収録」「公開資料では確認できず」などと明示し、市への確認や図書館での現物確認が必要な段階であることを、内部的な調査状況も含めてできる限り分かりやすく示すようにしています。0件という表示は、必ず「確認した結果ゼロ件だった」ことを意味し、「まだ調べていない」場合とは区別しています。
          </p>
          <p>
            実際にどこまで調査を終えているか（例：延岡市史の資料本文をページ単位でどこまで確認済みか）といった具体的な件数は、
            <Link to="/data-status" className="font-medium text-blue-700 underline hover:text-blue-800">
              データ収録状況ページ
            </Link>
            の「一次資料の調査状況」でご確認いただけます。
          </p>
        </Section>
