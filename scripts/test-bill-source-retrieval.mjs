...
assert.ok(Array.isArray(billVotes) && billVotes.length === 1206, `billVotes.jsonの件数が1,206件ではありません（${billVotes.length}件）`);
...
check("A + B + D の合計が議案総数1,206件と一致する（C区分は実例0件のため式に含めない）", () => {
  assert.equal(A + B + D, 1206, `A+B+Dの合計が1,206件ではありません（${A + B + D}件）`);
});
