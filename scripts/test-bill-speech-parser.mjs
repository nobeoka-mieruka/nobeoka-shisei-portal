...
check("Phase147で発見・修正したバグ：市道の路線認定・廃止・変更は、予告文（「議案第X号は、市道の路線認定であります。」）の次の文に路線名・延長等の具体的内容が来る実データパターンがあり、whatの1文目だけでなく次の文もreasonとして拾う（路線名はタイトルや番号からではなく本文からのみ抽出）", () => {
  const text =
    "次に、議案第二五号は、市道の路線認定であります。" +
    "今回認定いたします路線は、寄附採納による野田三〇号線でありまして、総延長五十四・〇メートルであります。" +
    "次に、議案第二六号は、延岡市避難行動要支援者名簿に関する条例の制定であります。";
  const blocks = splitSpeechIntoBillBlocks(text);
  const bill = blocks.get(25);
  const fields = extractCandidateFields(bill, "その他", "市道の路線認定（1路線）");
  assert.ok(fields.what.includes("路線認定"), "路線認定である旨がwhatに含まれていません");
  assert.ok(fields.reason?.includes("野田三〇号線") && fields.reason?.includes("五十四・〇メートル"), "路線名・延長がreasonへ抽出できていません（一次資料本文からの抽出のみが対象）");
  assert.ok(!fields.reason?.includes("避難行動要支援者"), "次の議案（第26号）の内容が混入しています（項目8：文脈混入の防止）");
});

check("Phase147で発見・修正したバグ：一括見出し（「議案第X号及び第Y号は」）の内訳を個別に述べる文で、「は」の直後に読点が無く地名等が直接続く表記（「まず、議案第◯号は◯◯において、」）でも、接頭語（次に/まず等）が明示されていれば個別マーカーとして検出できる（項目8：文脈混入の防止。接頭語が無い裸の表記までは拡張しない）", () => {
  const text =
    "次に、議案第二〇号及び第二一号は、辺地に係る総合整備計画の策定であります。" +
    "これらの議案は、辺地とその他の地域との著しい格差の是正を図ることを目的とした、辺地に係る公共的施設の総合整備のための財政上の特別措置等に関する法律第三条の規定に基づき、総合整備計画を策定するものであります。" +
    "まず、議案第二〇号は藤の木辺地において、農道小原潜水橋の補修工事を行うものであります。" +
    "次に、議案第二一号は松瀬辺地において、林道松瀬歌糸線の舗装補修工事を行うものであります。" +
    "次に、議案第二二号は、専決処分の承認であります。";
  const blocks = splitSpeechIntoBillBlocks(text);
  const b20 = blocks.get(20);
  const b21 = blocks.get(21);
  assert.ok(b20?.individualText?.includes("藤の木辺地"), "議案第20号固有の説明（藤の木辺地）が抽出できていません");
  assert.ok(b21?.individualText?.includes("松瀬辺地"), "議案第21号固有の説明（松瀬辺地）が抽出できていません");
  assert.ok(!b20.individualText.includes("松瀬辺地"), "議案第20号に議案第21号の内容が混入しています");
  assert.equal(b20.commonText, b21.commonText, "共通説明（辺地に係る総合整備計画の策定）が20号・21号で一致していません");
});

check("Phase147で発見・修正したバグ：一括見出し「議案第X号<件名>及び議案第Y号<件名>につきましては、」のように番号の直後に「及び」ではなく各議案の件名（条例名等）が挟まる表記でも一括マーカーとして検出し、直前の議案のブロックへ次の議案の内容が混入しない（項目8：文脈混入の防止）", () => {
  const text =
    "次に、議案第一〇号令和五年度延岡市下水道事業会計補正予算は、収益的収入及び支出におきまして、それぞれ六百万円を追加しております。" +
    "次に、議案第一一号延岡市一般職職員給与条例の一部改正及び議案第一二号職務に専念する義務の特例に関する条例の一部改正につきましては、新型コロナウイルスの五類移行に伴い、特例を廃止するものであります。" +
    "次に、議案第一四号は、延岡市火災予防条例の一部改正であります。";
  const markers = findBillMarkers(text);
  const and2 = markers.find((m) => m.kind === "and2");
  assert.ok(and2, "件名入りの一括マーカー（議案第11号及び第12号）が検出できていません");
  assert.deepEqual(and2.billNumbers, [11, 12], "件名入り一括マーカーの議案番号が正しくありません");
  const blocks = splitSpeechIntoBillBlocks(text);
  const bill10 = blocks.get(10);
  const fields10 = extractCandidateFields(bill10, "予算", "令和5年度延岡市下水道事業会計補正予算");
  assert.equal(fields10.reason, null, "議案第10号のreasonに議案第11号・12号の内容が混入しないはずがnullではありません");
  assert.ok(!fields10.what.includes("一般職職員給与条例"), "議案第10号のwhatに議案第11号の内容が混入しています");
});
