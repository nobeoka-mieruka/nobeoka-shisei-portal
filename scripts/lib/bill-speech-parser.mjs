...
// 決算議案は、審議される定例会自体は令和期でも、対象年度（決算は前年度分を報告する）が
// 平成期にまたがる場合がある（例：令和元年9月定例会の決算議案は「平成三十年度」の決算）。
// 「元」は漢数字の桁読み（KDIGIT）に含まれない特別な読みのため、別途許容する。
// 年号の年数（「三十年度」等）は、議案番号（KDIGIT、桁の読み下し）と異なり、通常の位取り
// 漢数字表記（十・百等の乗数を伴う）で書かれる（例：「平成三十年度」は3×10であり、
// 桁読みの「三〇」ではない）。そのため年号部分だけは「十」も許容する別の文字集合を使う。
const ERA_YEAR = `(?:令和|平成)(?:元|[〇一二三四五六七八九十]+)年度`;
...
export function findBillMarkers(text) {
  const patterns = [
    ...
    // Phase147で発見・修正：「議案第X号<件名>及び議案第Y号<件名>につきましては、」のように、
    // 番号のすぐ後に「及び」が来ず、間に各議案の件名（条例名等）が挟まる表記も実データで確認済み
    // （例：令和5年6月定例会「議案第一一号延岡市一般職職員給与条例の一部改正及び議案第一二号
    // 職務に専念する義務の特例に関する条例の一部改正につきましては、」）。この表記が未対応だったため
    // マーカーとして検出されず、直前の議案（例：議案第一〇号）のブロック境界が本来より後ろまで
    // 誤って広がり、無関係な後続議案の文章が混入していた（項目8：文脈混入の防止）。
    // 件名部分は句読点を含まない名詞句のみを許容し（budget/kessanパターンと同じ設計方針）、
    // 番号のみの従来表記（件名なし）は引き続き後方互換で検出される。
    {
      kind: "and2",
      re: new RegExp(
        `議案第(${KDIGIT})号[^、。]{0,40}?及び(?:議案第)?第?(${KDIGIT})号[^、。]{0,40}?(?:については|につきましても|につきまして|は)`,
        "g",
      ),
    },
    ...
    { kind: "kessan", re: new RegExp(`議案第(${KDIGIT})号(?:${ERA_YEAR})?延岡市[^、。]{0,24}?の決算額は、`, "g") },
    // 決算（企業会計の剰余金処分）専用：「議案第X号[令和○年度]延岡市○○剰余金の処分及び決算の認定について、」
    // （「の決算額は、」ではなく「について、説明申し上げます」で始まる表記を実データで確認済み）。
    { kind: "kessan_jouyokin", re: new RegExp(`議案第(${KDIGIT})号(?:${ERA_YEAR})?延岡市[^、。]{0,30}?剰余金の処分及び決算の認定について、`, "g") },
    ...
    { kind: "budget", re: new RegExp(`議案第(${KDIGIT})号(?:${ERA_YEAR})?延岡市[^、。]{0,30}?予算(?:の規模)?は`, "g") },
    ...
    // Phase147で発見・修正：一括見出し（and2等）の内訳を個別に述べる文で、「は」の直後にすぐ
    // 地名・対象等が続き、読点が省略される表記も実データで確認済み（例：令和3年6月定例会
    // 「まず、議案第二〇号は藤の木辺地において、農道小原潜水橋の補修工事を行うものであります。」）。
    // budgetパターンで既に採用している「「は」の直後の読点を必須にしない」設計と同じ理由。
    // ただし、この読点省略パターンは「次に、」「まず、」等の列挙を示す接頭語が明示されている場合
    // にのみ許容する（接頭語が無い裸の「議案第X号は」まで読点省略を許すと、他の議案の説明文中に
    // 副次的に現れる「議案第X号は…」を誤って新しい境界と誤検出するリスクが高まるため）。
    // 上の行（接頭語任意・読点必須）と一致するケースは、配列内でこちらより先に処理されるため
    // dedupで重複排除され、挙動は変化しない（読点が省略されている場合のみ、この行が新たに拾う）。
    {
      kind: "individual",
      re: new RegExp(`(?:次に、|まず、|初めに、|続きまして、)議案第(${KDIGIT})号(?:については|につきましても|につきまして|は)、?`, "g"),
    },
  ];
...
function guessAccountNameFromTitle(billTitle) {
  ...
  let name = billTitle
    .replace(/^(?:令和|平成)(?:元|[0-9〇一二三四五六七八九]+)年度/, "")
    .replace(/^延岡市/, "")
    .replace(/（[^）]*）$/, "")
    .replace(/(補正)?予算$/, "")
...
export function extractCandidateFields(block, category, billTitle) {
  ...
  // Phase147で発見・修正：市道の路線認定・廃止・変更は、「議案第X号は、市道の路線認定で
  // あります。」のような予告文だけがwhatの1文目に来て、路線名・延長等の具体的な内容は
  // 次の文に書かれている実データパターンを確認した（項目6：市道路線認定の重点確認）。
  // 路線名・起終点等は本文の語（「路線認定/路線廃止/路線変更」）そのものからのみ識別し、
  // 議案名や番号からは推測しない。詳細文が存在しない場合はreasonをnullのままにする（推測禁止）。
  if (category === "その他") {
    const roadSentenceIndex = sentences.findIndex((s) => /路線認定|路線廃止|路線変更/.test(s));
    if (roadSentenceIndex !== -1) {
      const detailSentence = sentences[roadSentenceIndex + 1] ?? null;
      return {
        what: sentences[roadSentenceIndex],
        reason: detailSentence,
        amountRawText: null,
        amountSentences,
      };
    }
