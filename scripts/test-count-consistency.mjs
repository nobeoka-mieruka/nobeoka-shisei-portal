 ...
 *
 * Phase262（TASK-191）：1,178 件 → 1,206 件。自動更新（2026-09-18 以降の「延岡市議会の最新資料を自動更新」
 * コミット）が、令和8年9月定例会（第27回）の審議結果PDF
 * （https://www.city.nobeoka.miyazaki.jp/uploaded/attachment/29070.pdf、令和8年9月18日現在）から
 * 議案第29〜47号・第49〜53号、意見書第9号・第10号、決議第10号の28件を追加した。2026-09-19 に同PDFを
 * 取得し、同会期の登録29件（Phase225の議案第48号を含む）すべてについて議案番号・件名・審議結果・議決日が
 * 一致することを確認した（件名は全角／半角数字の違いのみ。「原案同意」は抽出規則
...
check(`議案総数が実データ・軽量インデックス・検索索引で一致し、現在値（${BILL_TOTAL}件）である`, () => {
  ...
  assert.equal(
    ...
    `議案総数が${BILL_TOTAL}件ではありません（${bills.length}件）。` +
      "1,178件以下へ戻す変更は許されません（Phase262で令和8年9月定例会の28件を審議結果PDFと照合済み）。" +
