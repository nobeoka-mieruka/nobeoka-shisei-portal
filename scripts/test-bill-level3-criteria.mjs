...
check("1,206件全体の「原資料到達性区分（A/B/D）×説明品質段階（Level1/Level2/Level3）」クロス集計の合計が1,206件と一致する", () => {
  ...
  assert.equal(sum, 1206, `クロス集計の合計が1,206件ではありません（${sum}件）: ${JSON.stringify(matrix)}`);
  /*
