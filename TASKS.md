...
状態：IN_PROGRESS（単位3の2件目まで完了：令和3年12月定例会＝第17回、9月定例会＝第16回。残り2会期は未着手）
優先度：A
対象：`src/data/councilSpeechSummaries.json`、`src/data/councilSessions.json`、`src/data/formerMembers.json`
依存関係：TASK-005A・005Bの完了後
...
公式資料：延岡市議会会議録検索システム（発言者一覧・発言原文、令和3年第17回・第16回定例会）

完了記録：
- 完了日：2026-08-05
- コミットID：（後述）
- 変更概要：上記のとおり。会議録確認済み一般質問の累計が243件→255件→270件に増加（12月分+12、
  9月分+15）。単位3の残り2会期（6月＝第15回・3月＝第12回定例会）は次回以降登録する。
