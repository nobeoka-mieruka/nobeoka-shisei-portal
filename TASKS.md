...
状態：IN_PROGRESS（単位4の2件目まで完了：令和2年12月定例会＝第10回、9月定例会＝第9回。残り2会期＝6月＝第8回・3月＝第6回は未着手）
優先度：A
対象：`src/data/councilSpeechSummaries.json`、`src/data/councilSessions.json`、`src/data/formerMembers.json`、
...
公式資料：延岡市議会会議録検索システム（発言者一覧・発言原文、令和2年第10回・第9回定例会）

完了記録：
- 完了日：2026-08-05
- コミットID：（後述）
- 変更概要：上記のとおり。会議録確認済み一般質問の累計が301件→316件→333件に増加（12月分+15、
  9月分+17）。単位4の残り2会期（6月＝第8回・3月＝第6回定例会）は次回以降登録する。
