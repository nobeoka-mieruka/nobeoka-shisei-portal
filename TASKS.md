...
状態：DONE（令和4年度4定例会、令和4年6月・9月・12月定例会と令和5年3月定例会の直前3会期分。ただし
令和4年3月定例会＝第21回定例会は今回未着手のため次回以降に持ち越し、状態としては令和4年度分の
主要3定例会＝完全収録で区切りとした）
優先度：A
...
公式資料：延岡市議会会議録検索システム（発言者一覧・発言原文、令和4年第28回・第26回・第24回定例会）

完了記録（令和4年12月・9月・6月定例会分）：
- 完了日：2026-08-05
- コミットID：（後述）
- 変更概要：上記のとおり。会議録確認済み一般質問の累計が201→214→229件に増加（12月分+12、9月分+13、
  6月分+15）。令和4年3月定例会（第21回定例会）はTASK-005C継続分として次回登録する。
