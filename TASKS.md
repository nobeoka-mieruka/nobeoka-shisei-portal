...
### TASK-033 副市長・教育長・農業委員会委員ページ新設【フェーズ13・第1弾】

状態：IN_PROGRESS（副市長2名・教育長1名・農業委員会委員19名＝計22名を登録。監査委員・選挙管理委員は未確認）
優先度：C
...
- 既存の`billVotes.json`（議案等審議結果）に登録済みの人事議案（「○○の選任」「○○の任命」に対する
  ...
  - 延岡市公式ホームページ「令和8年度 経営体制」ページで、副市長・教育長の氏名が現在の体制と
    一致することも確認済み
- 監査委員は、複数の選任同意議案（中城あかね・甲斐行雄・比江島久美子・服部俊明・甲斐正幸・後藤博文の
  各氏）が確認できたが、現在何名で構成され、それぞれ誰が務めているかを確実に特定できる公式資料
  （委員名簿等）を確認できなかったため、誤った現職情報を掲載しないよう今回は掲載を見送った
...
- 確認できない役職（監査委員・選挙管理委員）は掲載を見送り、その理由をページ上に明記する（達成）
...
- 変更概要：上記のとおり。`validate:data`/`typecheck`/`lint`/`build`/`validate:seo`すべて成功
  （1251/1251ページ生成、SEO failures=0）。監査委員・選挙管理委員会の追加登録は今後の課題として残る。
