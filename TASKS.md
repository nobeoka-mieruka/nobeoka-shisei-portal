...
BLOCKED理由・再開条件（2026-08-06に既存データの健全性を再確認）：
- 延岡市単独の財政力指数・経常収支比率・実質収支（令和6年度決算、TASK-035で確定済み）は登録済みだが、
  比較対象となる宮崎市・都城市・日向市等の同年度・同条件の財政指標がまだ未収集
- 再開条件：総務省「地方財政状況調査（決算統計）」の市町村別データ、または各自治体が公表する
  「財政状況資料集」から、延岡市と同一年度（令和6年度決算）の財政力指数・経常収支比率・
  実質公債費比率・将来負担比率が確認できた場合
- 同じ検索を繰り返さないよう、今回は新規の探索は行っていない
