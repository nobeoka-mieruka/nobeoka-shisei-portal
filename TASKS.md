...
状態：IN_PROGRESS（単位3の1件目：令和3年12月定例会＝第17回定例会 完了。残り3会期は未着手）
優先度：A
対象：`src/data/councilSpeechSummaries.json`、`src/data/councilSessions.json`、`src/data/formerMembers.json`
依存関係：TASK-005A・005Bの完了後
...
公式資料：延岡市議会会議録検索システム（発言者一覧・発言原文、令和3年第17回定例会）

完了記録（令和3年12月定例会分）：
- 完了日：2026-08-05
- コミットID：（後述）
- 変更概要：上記のとおり。会議録確認済み一般質問の累計が243件→255件に増加（12件登壇、全て
  question-like）。残り3会期（9月＝第16回・6月＝第15回・3月＝第12回定例会）は次回以降登録する。
