...
2026-08-06追記（次回の着手候補としての手がかり）：
- 財政指標3項目（財政力指数・経常収支比率・実質収支、TASK-035で解消）を調査した際、
  延岡市「財務書類４表」（統一的な基準による地方公会計、`/uploaded/attachment/26656.pdf`、
  令和6年度決算分・20ページ）が`Read`ツール（`pages`指定なし）で正しくテキスト取得できる
  ことを確認した（前回セッションでは`pdftotext`でラベル判読不能とし不採用にしていたPDF）
- このPDFの貸借対照表（令和7年3月31日現在）に、一般会計等ベースの地方債残高
  （固定負債の地方債48,121百万円＋1年内償還予定地方債5,829百万円＝53,950百万円）が
  記載されている。ただし令和6年度末の1時点のみで、複数年度分の推移ではないため、
  本タスクの受入条件（年度ごとの残高区別）を満たすには過去年度分PDFの追加確認が必要
- 次回は、延岡市公式ホームページの財務書類４表公表ページから過去年度分（令和3〜5年度決算）
  のPDFを探し、同様に`Read`ツールで直接読み込むことで複数年度推移を確認できる可能性が高い
