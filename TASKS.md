...
状態：IN_PROGRESS（単位5の1件目完了：令和元年12月定例会＝第5回。残り2会期＝9月＝第3回・6月＝第2回は未着手）
優先度：A
対象：`src/data/councilSpeechSummaries.json`、`src/data/councilSessions.json`、`src/data/formerMembers.json`、
`src/data/archiveMemberProfiles.json`
依存関係：TASK-005A・005Bの完了後
...
公式資料：延岡市議会会議録検索システム（発言者一覧・発言原文、令和元年第5回定例会）

完了記録：
- 完了日：2026-08-05
- コミットID：（後述）
- 変更概要：上記のとおり。会議録確認済み一般質問の累計が354件→367件に増加（12月分+13）。単位5の
  残り2会期（9月＝第3回・6月＝第2回定例会）は次回以降登録する。
