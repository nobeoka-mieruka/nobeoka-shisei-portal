...
このファイルは、Phase197〜200 で固定し、Phase201〜242 の品質改善と、Phase243〜247 の差分更新・Phase248〜251 の改善・Phase252〜255 の監視自動化・Phase256〜258 の会議録照合を反映した安定版の記録です。
...
| 更新履歴エントリ | 137 | 152 | Phase202・208・225・235・236・239・244・248・256 の記録 |
...
| 更新履歴エントリ | 152 |
| 会期 | 62 |
| 検索索引エントリ | 2,308（うち固有URL 1,932） |
| 出典URL（重複除去） | 5,531 |
| プリレンダリング済みページ | 2,287 + `404.html` |
| route template | 79 |
| sitemap 掲載URL | 2,218 |
...
| --- | --- |
| page 間件数矛盾 | 0 |
| broken internal link | 0（2,287ページ / リンク先2,353種類を検査） |
...
| test failures | 0（1,673 checks / 39スクリプト。Phase256で会議録照合済み会期の回帰検査を追加） |
| validate:data errors | 0 |
| validate:seo failures | 0（2,288ページ） |
| validate:content errors | 0（2,288ページ） |
...
## Phase256〜258：令和8年6月定例会の会議録照合（2026-09-15）

Phase252 の監視が検出していた「原本は公開済み・当サイトとしては照合が未了」の状態を解消した回。
**新しい質問レコードは1件も作っていない**。質問通告書ベースで登録済みの既存14件へ、
会議録本文から確認できた内容を追記した。

### 会議録本文との照合（Phase256）

対象は令和8年6月定例会の一般質問14件（2026-06-23 5名／2026-06-24 5名／2026-06-25 4名）。
公式会議録検索システムの本会議録3日分（R080623A・R080624A・R080625A）の発言セグメントを
議員ごとに切り出し、**人が本文を読んで**質問項目・質問内容・答弁者・答弁内容・
再質問と再答弁の順序を照合した照合仕様（`data/minutes/june-specs/01〜14`）を作成。
そこから発言レコードを組み立てて公開データへ反映した。

| 項目 | 値 |
| --- | ---: |
| 対象質問 | 14件（14名） |
| 完全照合（質問項目150件すべて `questionAnswerLinkStatus: confirmed`） | 14件 |
| 部分照合 | 0件 |
| 未照合 | 0件 |
| 登録した質問項目 | 150件 |
| 質問・答弁のやり取り（exchanges） | 625件（うち再質問・再答弁 325件） |
| 出典（会議録の発言セグメント単位） | 510件 |
| 新規に作成した質問レコード | 0件 |

要約は**原文に書かれている事実のみ**から作成し、原文の転載はしていない。
答弁の本文はすべて出典URL（発言セグメント単位の会議録ページ）から参照できる。

**通告と実際の発言の差**：稲田雅之議員は通告13項目のうち「新副市長」の1項目を、
前日の一般質問で答弁があったことを理由に**本人が質問の冒頭で割愛を申し出ている**。
この1項目は質問・答弁が行われていないため発言レコードへ含めず、
通告書ベースの項目一覧はそのまま残したうえで、その事実を注記へ明記した。
「日付が過ぎたから実施済み」という扱いはしていない。

**表示の切り替え**：会議録リンク（`transcriptUrl`）を持つ質問は「予定質問」ではなくなるため、
質問詳細ページの「市の答弁（要約）」が「確認できていない」と表示し続けないようにし、
確認日のラベルも切り替えた。9月定例会の13件は従来表示のまま。

**収録状況**：`questionCollectionStatus.json` の2026-06を
`transcriptAvailable: true`／`registeredSpeakerCount: 14`／`registeredQuestionCount: 150` へ更新した。
これに伴い、会期IDを直接書いていた回帰テスト3件を、
収録状況（`transcriptAvailable`）から対象を導出する形へ書き換えている
（次に照合が進む会期でも壊れないようにするため）。確認済み一般質問は418件→432件、
質問項目は1,567件→1,717件。

### dry-run専用botブランチの整理（Phase257）

Phase253 で増殖は止めたが、過去に残っていた19本のブランチが未処理だった。
削除前に復元情報（HEAD/tree SHA・変更ファイル・blob SHA・main包含の有無）を
`reports/phase257-dryrun-branch-cleanup.json` へ保存したうえで整理した。

| 項目 | 値 |
| --- | ---: |
| 開始時のブランチ数 | 19（すべて `bot/auto-update-dryrun-*`） |
| 削除 | 19 |
| 残存 | 0（`main` のみ） |
| force delete | 0 |
| open PR | 0 |

いずれも `reports/auto-update/` 配下のdry-run記録のみを含み、
`src/data`・コード・一次資料の未反映コミットは1件も無かった。

### 令和8年9月定例会の会議録確認（Phase258）

`npm run watch:question-transcripts` を実行。質問13件の会期（2026-09）は
`WAITING_FOR_OFFICIAL_RECORD` のままで、**DATA CHANGE = 0**。
質問日を過ぎていることを根拠に実施済みへ変更することはしない。
